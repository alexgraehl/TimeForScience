...
foreach my $ff (@mapFiles) {
    ...
    my $PAD_OUT_FILES_INTO_RECTANGLE_WITH_NA = " table-no-ragged.py --pad=NA "; # This is Alex's python script!
    ...
    agwSystem(qq{join.pl -1 4 -2 1 -o "NO_ANNOTATION_AVAILABLE" $AAA $toPrimary | $PAD_OUT_FILES_INTO_RECTANGLE_WITH_NA > $BBB }); # replace the species-specific Ensembl Gene ID field with the literal text "Ensembl Gene ID"
    ...
    agwSystem(qq{join.pl -1 2 -2 1 -o "NO_ANNOTATION_AVAILABLE" $BBB $fromPrimary | $PAD_OUT_FILES_INTO_RECTANGLE_WITH_NA > $CCC});
