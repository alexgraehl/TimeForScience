...
$registry->load_registry_from_db( '-host'   => 'useastdb.ensembl.org' #'ensembldb.ensembl.org', #'ensembldb.ensembl.org',
				  , '-port' => '5306' # 5306 on useastdb means: Current ensembl version with Previous ensembl version ONLY
                                  , '-user' => 'anonymous');
#                                  '-db_version' => '68' ); # 70 now


my $genome_db_adaptor = Bio::EnsEMBL::Registry->get_adaptor('Multi', 'compara', 'GenomeDB');

my $all_genome_db_array_ptr = $genome_db_adaptor->fetch_all();
print "### The following species existed: ";
foreach my $this_genome_db (@{$all_genome_db_array_ptr}) {
    print $this_genome_db->name . ", ";
}
print "\n";

# first you have to get a Member object. In case of homology it is a gene, in case of family it can be a gene or a protein

my $member_adaptor = Bio::EnsEMBL::Registry->get_adaptor('Multi', 'compara', 'Member');
my @membersToGet = ($member_adaptor->fetch_by_source_stable_id('ENSEMBLGENE', 'ENSG00000222139') ); # just getting them for this one gene right now!

for my $member (@membersToGet) {

    print "CHR: " . $member->chr_name() . ":" . $member->chr_start() . "-" . $member->chr_end(). "\t" . $member->description . "\n";
...
# That will return a reference to an array with all homologies (orthologues in other species and paralogues in the same one)
# Then for each homology, you can get all the Members implicated
	...
	my $homolog_member_attributes_array_ptr = $homology->get_all_Member_Attribute();
	foreach my $homolog_attr_array_ptr (@$homolog_member_attributes_array_ptr) {
	    # See docs for Member at http://www.ensembl.org/info/docs/Doxygen/compara-api/classBio_1_1EnsEMBL_1_1Compara_1_1Member.html
	    my ($hmem, $hattr) = @$homolog_attr_array_ptr;
	    # Attribtues are documented here: http://uswest.ensembl.org/info/docs/Doxygen/compara-api/classBio_1_1EnsEMBL_1_1Compara_1_1Attribute.html
	    print "DESCRIPTION/SUBTYPE: " . $homology->description . " " . $homology->subtype . "\n";


	    my $hg  = $hmem->get_Gene();
	    my $htr = $hmem->get_Transcript();
	    
	    print join("\t", $hmem->genome_db()->name()
		       , ("SRCNAME::::" . $hmem->source_name())
		       , ("STABLEID::::" . $hmem->stable_id())
#		   , $hmem->common_name
		       , ("DESC::::" . $hmem->description() )
		       , ($hmem->chr_name . ":" . $hmem->chr_start . "-" . $hmem->chr_end )
		       , ("STRAND::::" . $hmem->chr_strand)
		       , ("LABEL::::" . $hmem->display_label)     # See docs for Gene at http://www.ensembl.org/info/docs/Doxygen/core-api/classBio_1_1EnsEMBL_1_1Gene.html 
		       , defined($htr) ? $htr->biotype : 'NG'  
		       , defined($htr) ? $htr->stable_id : 'NG'  
		       , defined($htr) ? $htr->length : 'NG'  
		) . "\n---------------------\n";
	    print "CHR: " . $hmem->chr_name() . ":" . $hmem->chr_start() . "-" . $hmem->chr_end(). "\t" . $hmem->description . "\n";
	}
	$hcounter++;
	#if ($hcounter >= 100) { die "ok that's enough testing for now! Debugging done! Remove this statement later!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"; }
    }

    # And if they are defined dN and dS related values
    
#    my $homologue_genes = $homology->gene_list();
#    print "HERE ARE THE GENES: \n";
#    print join(" and ", @$homologue_genes), " are ", $homology->description, "\n";
