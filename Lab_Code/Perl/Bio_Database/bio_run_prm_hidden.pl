...
#
#    BRPH_em_acyclicity_check = if true then we perform acyclicity checks during EM
#
...
use lib "$ENV{MYPERLDIR}/lib"; use lib "$ENV{TIME_FOR_SCIENCE_DIR}/Lab_Code/Perl"; require "bio_execute.pl";
use lib "$ENV{MYPERLDIR}/lib"; use lib "$ENV{TIME_FOR_SCIENCE_DIR}/Lab_Code/Perl"; require "bio_system.pl";
use lib "$ENV{MYPERLDIR}/lib"; use lib "$ENV{TIME_FOR_SCIENCE_DIR}/Lab_Code/Perl"; require "bio_change_attribute_in_file.pl";
...
my $em_acyclicity_check = $settings{"BRPH_em_acyclicity_check"};
...
#----------------------------------------------------------------
# remove_scores_from_file
#----------------------------------------------------------------
sub remove_scores_from_file
{
  my $file = $_[0];
...
sub run_em_hide_attribute
{
  ...
  if ($em_acyclicity_check eq "true")
  {
    my $backup_file = "$dir/out/${prefix}_l$iteration.out.bak.$r";
    my $tsc_file = "$dir/out/${prefix}_l$iteration.tsc";
    my $outfile = "$dir/out/${prefix}_l$iteration.out";
    execute("cp $outfile $backup_file", $verbose);
    remove_scores_from_file($outfile);
    execute("bio_run_prm.pl $gene_table -m -i $iteration", $verbose);
    execute("mv $backup_file $outfile", $verbose);

    my $hide_attribute = "$hide_attributes[$group_idx_to_hide][0]";
    my $hide_print_attribute = "$hide_attributes[$group_idx_to_hide][0]_i$next_iteration";

    execute("gx2acyclicity.pl $tsc_file -g $gene_table -d $database -a $hide_attribute -p hidden_$hide_print_attribute -o $dir/em_iter_acyclicity.dat", $verbose);

    print META_FILE "SoftEvidenceFile=em_iter_acyclicity.dat\n";
  }

  # RUN EM
  execute("cd $dir; $ENV{HOME}/develop/frog_linux_release/SamplePrograms/prm_param_learn1 ${prefix}_l$iteration.meta", $verbose);
...
#----------------------------------------------------------------
# change setting (check list for in-middle start --
#    * hide copying the file
#    * hide deleting the scores file
#    * hide prepare_sql_for_next_iteration
#    * change i in the hide groups to whatever you want
#----------------------------------------------------------------
...
for (my $round = 0; $round < $round_robin; $round++)
{
  for (my $i = 0; $i < $hide_groups; $i++)
  {
    ...
    if ($hide || $exp_level_time_series_length > 0)
    {
