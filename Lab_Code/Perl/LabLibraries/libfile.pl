...
sub readMapping($$$;$) {
    # Arg 1: filename
    # Arg 2: first column index, counting from 1 and not 0!!!!
    # Arg 3: second column index, counting from 1 and not 0!!!!
    # Arg 4 (Optional): Column delimiter (Default: tab)
    # Assumes tab-delimiting
    # Returns a hash of the item in col1 mapped to the item in col2
    # The way you use this function is:
    # my %endmap = %{readMapping("filename", 3, 6)}; # col3 = key, col6=value
    my ($filename, $col1, $col2, $delim) = @_;
	...
	my $elem1 = $line[$col1 - 1];
	my $elem2 = $line[$col2 - 1];
	if (exists($map{$elem1}) && defined($map{$elem1})) {
	    if ($map{$elem1} ne $elem2) {
		# Disagreeing mapping. Have to quit here, sorry.
		die "PROBLEM: Duplicate but non-agreeing mapping detected for element $elem1.\n"
		    . "Element <$elem1> was formerly <$map{$elem1}>,\n"
		    . "but we are trying to set it to the new value of <$elem2>.\n"
		    . "Probably this just means that the key/value pair you chose to map is not suitable for uniquely mapping key->value.\n";
	    } else {
		# looks like it's just a duplicate entry, no big deal
	    }
	} else {
	    $map{$elem1} = $elem2; # Add a new mapping
	}
    }
    close(FILE) or die "Cannot close file $filename. $!";
    $map{"AGWTest"} = "AGWTested";

    return \%map; # return a reference!
    # For the way to use the reference, look at the top of this function.
}
...
#-----------------------------------------------------------------------------
sub readFileColumns {
    # this may be a dupe of "getCols," which is below
    ...
    return \@tuples;  # <-- Returns a REFERENCE (note! multi-dimensional array!)
}
...
# my @resultColumnArray = @{readFileColumn(filename, 0, "\t")};
...
# delimiters.
#-----------------------------------------------------------------------------
# \@ARRAY readFileColumn(FILE_NAME="-", INDEX=0, DELIMITER="\t")
#-----------------------------------------------------------------------------
sub readFileColumn {
    my ($fileName, $column, $delim) = @_;
    $fileName = defined($fileName) ?  $fileName : '-' ; # <-- STDIN
    ...
    return \@lines; # <-- Returns a REFERENCE (single-dimensional array)
}
...
# Returns: a REFERENCE to an array.
#-----------------------------------------------------------------------------
# \@ARRAY readNumLinesFromFile(FILEHANDLE, integer)
#-----------------------------------------------------------------------------
...
# AGW: Note: is this really supposed to be "printFilep" and not "printFile???"
sub printFilep
{
...
sub numCols
{
    my ($filename, $delim, $eaten) = @_;
    $filename  = defined($filename)  ? $filename  : '-';
    $delim = defined($delim) ? $delim : "\t";

    my $FILEPTR;
    open($FILEPTR, $filename) or die("Could not open file '$filename'");
    my $num_cols = &numCols2($FILEPTR, $delim, $eaten);
...
#------------------------------------------------------------------------
# \@ARRAY readDataMatrix(FILENAME, ???, DELIMITER=TAB, ???)
#------------------------------------------------------------------------
# Reads a data matrix.
# Example usage:
#  my @arr = @{readDataMatrix("FILENAME.tab")};
#  foreach my $line (@arr) {
#      print join(" <tab> ", @{$line});
#      print "\n";
#  }
sub readDataMatrix
{
    my ($filename, $key_col, $delim, $max_cols) = @_;
    $key_col = defined($key_col) ? $key_col : 0;
    $delim   = defined($delim) ? $delim : "\t";

    my $fp = &openFile($filename);
    ...
    if(defined($max_cols)) {
	$$max_cols = $max; # <-- somehow this is a pointer (note the $$)
    }
...
sub appendToFilename($$) {
	...
	if (length($filename) > 256) {
	    my $shorterFilename = substr($filename, 0, 250);
	    $shorterFilename =~ s/\n/_NEWLINE_/g;
	    $shorterFilename =~ s/[\t ]+/ /g;
	    $shorterFilename =~ s/[\t ]+$//g;
	    $errorMessage .= "Cannot open file <$shorterFilename......> for appending!\n\nNOTE: The length of the filename was extremely long, so maybe you switched the order of the arguments to appendToFilename. The first argument is the FILENAME, the second is the string to be written.\n\n";
	} else {
	    $errorMessage .= "Cannot open file <$filename> for appending!\n";
	}
	die $errorMessage;
    }
    print FILE $string;
