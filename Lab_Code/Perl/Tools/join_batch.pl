...
if(scalar(@data_files) > 0) {
   for(my $i = 0; $i < scalar(@data_files); $i++) {
      ...
      system("tail +$tail $data_files[$i] | $maybeDos2UnixPipe cut.pl -f $key_cols[$i] | sed 's/\t/@@@@/g' >> $tmpFile2");
   }
...
if(scalar(@data_files) > 1) {
   for(my $i = 0; $i < scalar(@data_files); $i++) {
      ...
      my $tail = $head_lines[$i] + 1;
      system("tail +$tail $data_files[$i] | $maybeDos2UnixPipe cut.pl -f $key_cols[$i] | sed 's/\t/@@@@/g' > $tmpFile2");
      system("tail +$tail $data_files[$i] | $maybeDos2UnixPipe cut.pl -f $data_cols[$i] | paste $tmpFile2 - >> $tmpFile3");
