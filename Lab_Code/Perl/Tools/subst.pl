...
foreach my $file (@files)
{
   ...
   while(<$filep>)
   {
	 ...
	 for(my $i = 0; $i < scalar(@replacements); $i++)
	 {
	    ...
	    my $regex = $regexs[$i];
	    my $safe  = $regex; # <-- changed from the version below. Previously the version below prevented the program from running.
	    #my $safe  = $pad . $regex . $pad; # <-- what the heck is "$pad"? (it is a function in libfile, but that probably is not what's going on here
	    s/$regex/$safe/ge;
	    s/$safe/$repl/ge;
	 }
      }
