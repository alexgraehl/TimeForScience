...
# Deprecated! Use enrich.R instead.
# (note: enrich.R has different input files types, so there may still be reasons to use this program.)
# (enrich.R makes use of the internal R libraries for calculating wilcoxon, and has additional options.)
# enrich.R has more options and is more likely to be bug-free as a result of using R's internal libraries.
...
if (@argv!=2) {
  ...
  printf STDERR "wilcoxon.pl is deprecated. Use enrich.R for any future programs.\n";
  exit;
}
