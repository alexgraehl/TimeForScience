...
def handleCommandLineOptions():
    ...
    parser = optparse.OptionParser(
        USAGE,
        version="%prog version 1.0, generates a tabular-style file with the same number of elements in each line, when given a (possibly) ragged-edged file. You can change the input/output delimiter with --delim=<DELIM>. You can change what text gets used for padding out empty cells with --pad=<PADDING>.",
    )
    parser.add_option(
        "-d",
        "--delim",
        ...
        default="\t",
        type="string",
        help="Specify the delimiter character. Default is a <tab>.",
    )
    parser.add_option(
        "-p",
        "--pad",
        ...
        default="",
        type="string",
        help="Specify the text to print in a padded-out cell. Default is nothing. 'NA' is a popular option.",
    )
    (globalOptions, globalArgs) = parser.parse_args()

    # pdb.set_trace()
    for attr, value in globalOptions.__dict__.items():
        # print attr, value
        pass

    if len(globalArgs) < 1:
        sys.stderr.write(
            "table-no-ragged.py STDERR message: Since no filenames were specified, we are reading from STDIN.\n"
        )
...
if __name__ == "__main__":
    ...
    readingFromSTDIN = (len(globalArgs) == 0) or (
        globalArgs[0] == "-"
    )  ## "is a filename present on the command line? If not, then we're reading from STDIN"
    ...
    if readingFromSTDIN:
        if os.access(TEMPFILENAME, os.W_OK):
            ...
            sys.stderr.write(
                "WARNING: Could NOT remove the temp file we generated named " + TEMPFILENAME + " !\n"
            )
            pass
        pass
