...
Example command to make a heatmap from a large matrix file named INPUT_MATRIX.tab:

   plot.R -out-file OUTPUT-HEATMAP.pdf   -w 20 -h 10  -t \"MainTitle\" \\
                 heatmap   --uncentered  --range \"c(-10,10)\" \\
                 --missing \"orange\" \\
                 --red   \"c(0.2,0.0)\" --green \"c(1.0,0.0)\" --blue  \"c(0.4,0.4)\" \\
                 INPUT_MATRIX.tab
...
Available modules (for details use --help, like this:  plot.R heatmap --help):
  ...
  Important Note: Headers are ASSUMED for every file. You must also specify the number of header lines for EVERY file. So you should call \"scatter -h 0\" if you have no header lines in your scatterplot source file.
...
Global Options:",
                    ...
                    "log=s               use logarithm axes. Use 'x' or 'y' for single axes, 'xy' for both.",
                    ...
                    "height|h=f          height of plot area (inches). Default is probably 5. Note that -h means *num_headers* if it comes after the plot type is specified.",
...
datafile.options <- c("header|h=i    number of header lines. Default is 1. This is a plot-specific option. (-h means *height* if it comes before the plot type, but number of headers if it comes after)",
...
",
               ...
               "font=li font, as in par(font)",
               "size=lf font size multiplier (i.e. 1 is normal size)"
               )
  ...
  if (is.null(o$font)) o$font = par("font")

  result$plotFun <- function() {
    mtext(o$argv, at=o$at, side=o$side, line=o$line, adj=o$adj,
          font=o$font, cex=o$size)
  }
...
Takes one file argument.", sep="\n"),
  ...
  if (length(o$argv) > 1) {
    ...
    mai <- rep(NA, 4)
    mai[o$side] <- max(strwidth(lbl, units="inches")) + par("csi")*(o$gap+0.1)
...
Example Usage:
  plot.R prcurve --color \"red,green,blue\" --avgcolor black my.aucs",
               ...
               "truncateStart  start the line at the first observed recall point rather than at (0,1)",
...
plot.default(NULL, xlim=xlim, ylim=ylim, type='n', xlab='', ylab='',
             frame.plot=FALSE, axes=FALSE, log=globalOpt$log)
